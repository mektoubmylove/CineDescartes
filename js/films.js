...
function shuffleArray(array) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Activation du toggle dark/light
    ...
    // Récupération des éléments de la grille et de l'overlay
    const gridContainer = document.getElementById('films-grid');
    const overlay = document.getElementById('film-overlay');
    const overlayPoster = document.getElementById('overlay-poster');
    const overlayTitle = document.getElementById('overlay-title');
  
    // Génération dynamique de la grille des films
    shuffledMovies.forEach(movie => {
      const cell = document.createElement('div');
      cell.classList.add('film-cell');
      cell.setAttribute('data-id', movie.id);
  
      // Création de l'image du poster (initialement masquée)
      const img = document.createElement('img');
      img.src = movie.poster;
      img.alt = movie.title;
      img.style.opacity = 0; // Le poster est caché par défaut
  
      // Création d'un placeholder visible
      const placeholder = document.createElement('div');
      placeholder.classList.add('placeholder');
      placeholder.innerText = "Cliquez pour révéler";
    ...
    // Fonction pour afficher l'overlay avec le poster en grand et le titre
    function showOverlay(movie) {
      overlayPoster.src = movie.poster;
      // Le titre est maintenant encapsulé dans un lien qui redirige vers "movie.html?id=<movie.id>"
      overlayTitle.innerHTML = `<a href="movie.html?id=${movie.id}" style="color: inherit; text-decoration: none;">${movie.title}</a>`;
    ...
    // Ajout d'un écouteur de clic sur chaque cellule de film
    document.querySelectorAll('.film-cell').forEach(cell => {
      cell.addEventListener('click', () => {
        const movieId = parseInt(cell.getAttribute('data-id'));
        const movie = movies.find(m => m.id === movieId);
        if (movie) {
          // Révèle le poster dans la cellule
          ...
          // Affiche l'overlay avec le poster en grand et le titre
          showOverlay(movie);
        }
      });
    });
  
    // Permet de masquer l'overlay en cliquant sur le fond (hors image et titre)
